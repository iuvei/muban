  ...
  methods: {
    ...
    getGameUrl(item, isFun, device) {
      ...
      switch (item.category) {
        ...
        case 'MGS':
          if (device == 'mobile') {
            url = `/game/v2/gameMGS.php?gameId=${item.id}&isfun=${isFunString}&deviceType=mobile&lobbyUrl=${localUrl}`
          } else {
            url = `/game/v2/gameMGS.php?gameId=${item.id}&isfun=${isFunString}&deviceType=${item.subType == 'H5' ? 'h5' : 'pc'}&lobbyUrl=${localUrl}`
          }
          break
        case 'PNG':
          if (device == 'mobile') {
