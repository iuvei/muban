<template>
  <footer class="footer" v-if="showFooter">
    <router-link :to="{name:'index'}">
      <i class="iconyg yg-home"></i>
      <div>首页</div>
    </router-link>
    <router-link :to="{name:'preferential'}">
      <i class="iconyg yg-youhuiquanicon"></i>
      <div>优惠</div>
    </router-link>
    
    <router-link :to="{name:'slotgame'}"> 
      <i class="iconyg yg-94"></i>
      <div>游戏</div>
    </router-link>
    <!-- <router-link :to="{name:'cooperation'}">
      <i class="iconyg yg-daili"></i>
      <div>代理</div>
    </router-link> -->
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      account() {
        return this.isAgent ? {name: "agent_index"} : {name: 'user_index'};
      }
    }
  };
</script>
