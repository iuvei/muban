<template>
  <div class="pay-update">
    ...
    <input-inline label="新密码">
      <input ref="pwd1" v-model="pwdForm.new_password" type="password" placeholder="6位数字" maxlength="6">
      <eyes :dom="$refs.pwd1"></eyes>
    </input-inline>
    <input-inline label="重复新密码">
      <input ref="pwd1" v-model="pwdForm.confirm_password" type="password" placeholder="6位数字" maxlength="6">
      <eyes :dom="$refs.pwd1"></eyes>
    </input-inline>
    <v-button @click="submit">确定</v-button>
    <router-link :to="{name:'paypwd',params:{type:'2'}}" class="cz">重置支付密码</router-link>
  </div>
</template>
<script>
