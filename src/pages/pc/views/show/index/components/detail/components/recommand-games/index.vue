<template>
  <div>
    <div class="cfx game-title">
      <h2 style="padding-top: 50px;: ">最高人气游戏</h2>
      <ul class="game-menu fl">
        <li v-for="(item,i) in platformData" :key="item.name" :class="{active:active==item.value}" @click="active=item.value">
    ...
    <ul class="game-list game-content">
...
<style spcoed>
  ...
  .game-menu-more {
    margin-top: 21px;
    line-height: 49px;
    background: url(~pc/assets/images/more.png) no-repeat right;
    font-size: 14px;
    padding-right: 28px;
  }
  .game-title{
  ...
  .game-list li .star-info {
    ...
    height: 11px;
    width: 55px;
    background: url(~pc/assets/images/xing.png) repeat-x;
  }
