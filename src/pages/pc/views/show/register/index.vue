...
<script>
  ...
  export default {
    ...
    methods: {
      checked() {
        if (!check.account.test(this.data.account)) window.toast("登入账号的长度请介于6-10字符之间！");
        else if (!check.password.test(this.data.password)) window.toast("密码需为6-16位，包含数字和字母");
        else if (!check.phone.test(this.data.phone)) window.toast("手机号格式不正确");
        else if (this.data.imageCode == "") window.toast("请输入验证码");
        else {
          return true;
        }
      },
