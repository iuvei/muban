<template>
  <!-- <div class="fliter content">
    ...
    <fliter-input class="filter-input"></fliter-input>
  </div> -->
  <div class="fliter">
    <div class="select">
      <div class="contain cfx">
        <div class="select_tab fl">
          <div class="rows" v-for="(filter,i) in data" :key="filter.title">
            <span>{{filter.title}}</span>
            <span class="tabs" v-for="(item,i) in filter.data" :key="item.name" :class="['filter-item',{active:item.value==filter.active}]" @click="search(item,i,filter)">
              {{item.name}}
            </span>
          </div>
          <slot>搜索</slot>
